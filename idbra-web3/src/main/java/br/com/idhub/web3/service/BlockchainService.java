...
public class BlockchainService {
    ...
    public CompletableFuture<TransactionReceipt> publishStatusList(
        ...
        Credentials credentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));
    ...
    /**
     * Check if a wallet has issuer role - versão simplificada usando valor
     * constante
     */
    ...
    private CompletableFuture<TransactionReceipt> sendTransaction(
        ...
        org.web3j.protocol.core.methods.response.EthSendTransaction ethSendTransaction = web3j
    ...
    public CompletableFuture<TransactionReceipt> grantIssuerRole(String targetWalletAddress) throws Exception {
        try {
            Credentials adminCredentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));
     ...
     */
    private CompletableFuture<TransactionReceipt> grantIssuerRoleForContract(String targetWalletAddress,
        ...
        try {
            ...
            throw new RuntimeException(
                    "Erro ao conceder ISSUER_ROLE para contrato " + contractAddress + ": " + e.getMessage(), e);
        }
    }
     ...
     */
    public CompletableFuture<TransactionReceipt> grantIssuerRole(String targetWalletAddress, String adminWalletAddress)
            throws Exception {
     ...
     */
    private CompletableFuture<TransactionReceipt> waitForTransactionReceipt(String txHash, int maxAttempts,
            long intervalMs) {
     ...
     */
    public Optional<Map<String, Object>> getIdentityInfo(String identity) {
        try {
            ...
            // Retornar Map simples em vez de IdentityInfo
            String result = response.getValue();
     ...
     */
    public Optional<Map<String, Object>> getCredentialRevocation(String credentialId) {
        try {
                ...
                if (reasonOffset > 0) {
                    ...
                    if (reasonStart < hexData.length()) {
                        ...
                        if (reasonLength > 0 && reasonStart + 64 + (reasonLength * 2) <= hexData.length()) {
                            String reasonHex = hexData.substring(reasonStart + 64,
                                    reasonStart + 64 + (reasonLength * 2));
                            reason = new String(Numeric.hexStringToByteArray(reasonHex));
                        }
                    }
                }

                // Create Map with decoded data instead of RevocationRecord
    ...
    /**
     * Get system metrics from blockchain
     */
    public Map<String, Object> getSystemMetrics() {
        try {
            ...
            // Decode response based on ABI structure
            String result = response.getValue();

            // For now, return the raw result with decoded structure
            // TODO: Implement proper ABI decoding for getSystemMetrics return values
            return Map.of(
                    "totalDIDCount", 0,
                    "totalVerifiedDIDCount", 0,
                    "totalCredentialCount", 0,
                    "totalRevokedCredentialCount", 0,
                    "totalOperationCount", 0,
                    "rawResponse", result,
                    "timestamp", System.currentTimeMillis(),
                    "status", "success",
                    "message", "Métricas obtidas da blockchain (valores decodificados em desenvolvimento)");
    ...
    public boolean hasIssuerRoleForContract(String contractAddress, String walletAddress) {
        try {
            ...
            org.web3j.protocol.core.methods.request.Transaction hasRoleTransaction = org.web3j.protocol.core.methods.request.Transaction
            ...
            if (hasRoleCall.hasError()) {
                throw new RuntimeException("Erro ao verificar hasRole no contrato " + contractAddress + ": "
                        + hasRoleCall.getError().getMessage());
            }
            ...
            throw new RuntimeException(
                    "Erro ao verificar role de issuer para contrato " + contractAddress + ": " + e.getMessage(), e);
        }
    }
    ...
    // Adicionar método auxiliar para criar Credentials
     ...
     */
    public boolean hasRole(String role, String walletAddress) {
        try {
            // Use the existing method with ISSUER_ROLE for now
            String functionData = contractService.getHasRoleFunctionData(walletAddress);
            ...
            throw new RuntimeException("Erro ao verificar role " + role + ": " + e.getMessage(), e);
        }
    }

    /**
     * Grant a specific role to a wallet
     */
    public CompletableFuture<TransactionReceipt> grantRole(String role, String walletAddress) throws Exception {
        try {
            Credentials adminCredentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));

            // Use the existing method for now
            String functionData = contractService.getGrantRoleFunctionData(walletAddress);

            return sendTransaction(adminCredentials, functionData, didRegistryAddress);
        } catch (Exception e) {
            throw new RuntimeException("Erro ao conceder role " + role + ": " + e.getMessage(), e);
        }
    }

    /**
     * Revoke a specific role from a wallet
     */
    public CompletableFuture<TransactionReceipt> revokeRole(String role, String walletAddress) throws Exception {
        try {
            Credentials adminCredentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));

            // Use revokeAttribute for now as a workaround
            String functionData = contractService.revokeAttributeFunctionData(
                    walletAddress, "role", role);

            return sendTransaction(adminCredentials, functionData, didRegistryAddress);
        } catch (Exception e) {
            throw new RuntimeException("Erro ao revogar role " + role + ": " + e.getMessage(), e);
        }
    }
    ...
    /**
     * Set an attribute for an identity
     */
    public CompletableFuture<TransactionReceipt> setAttribute(
            String identity,
            String name,
            String value,
            Long validity) throws Exception {

        String functionData = contractService.setAttributeFunctionData(identity, name, value, validity);
        Credentials credentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));
        return sendTransaction(credentials, functionData, didRegistryAddress);
    }
    ...
    /**
     * Check if delegate is valid (alias for isValidDelegate)
     */
    public boolean validDelegate(String identity, String delegateType, String delegate) {
        return isValidDelegate(identity, delegateType, delegate);
    }

    // ===== CREDENTIAL MANAGEMENT FUNCTIONS =====

    /**
     * Revoke a credential (simplified version)
     */
    public CompletableFuture<TransactionReceipt> revokeCredential(
            String credentialId,
            String subject,
        ...
        String functionData = contractService.revokeCredentialFunctionData(credentialId, subject, reason);
        Credentials credentials = getCredentialsFromPrivateKey(System.getenv("ADMIN_PRIVATE_KEY"));
        return sendTransaction(credentials, functionData, didRegistryAddress);
    }
     ...
     */
    public Map<String, Object> getIdentityCredentials(String identity) {
        try {
            // Use a simple call for now
            String functionData = contractService.getCredentialRevocationFunctionData("dummy");
            ...
            if (response.hasError()) {
                System.out.println("Error getting identity credentials: " + response.getError().getMessage());
                return Map.of("error", "Failed to get credentials");
            }

            String result = response.getValue();
            // TODO: Implement proper decoding of bytes32[] array
            return Map.of(
                    "identity", identity,
                    "credentials", result,
                    "timestamp", System.currentTimeMillis());
            ...
            System.out.println("Error getting identity credentials: " + e.getMessage());
    ...
    public boolean isPaused() {
        try {
            ...
            return false; // TODO: Implement proper pause check
        } catch (Exception e) {
            System.out.println("Error checking pause status: " + e.getMessage());
            return false;
        }
    }
    ...
    public BigInteger getTotalDIDs() {
        try {
            ...
            return BigInteger.valueOf(0); // TODO: Implement proper total DIDs check
        } catch (Exception e) {
            System.out.println("Error getting total DIDs: " + e.getMessage());
    ...
    public BigInteger getTotalCredentials() {
        try {
            // Return a default value for now
            return BigInteger.valueOf(0); // TODO: Implement proper total credentials check
        } catch (Exception e) {
            System.out.println("Error getting total credentials: " + e.getMessage());
            return BigInteger.ZERO;
        }
    }

    /**
     * Get total revoked credentials
     */
    public BigInteger getTotalRevokedCredentials() {
        try {
            // Return a default value for now
            return BigInteger.valueOf(0); // TODO: Implement proper total revoked credentials check
        } catch (Exception e) {
            System.out.println("Error getting total revoked credentials: " + e.getMessage());
            return BigInteger.ZERO;
        }
    }
    ...
    public BigInteger getTotalOperations() {
        try {
            // Return a default value for now
            return BigInteger.valueOf(0); // TODO: Implement proper total operations check
        } catch (Exception e) {
            System.out.println("Error getting total operations: " + e.getMessage());
            return BigInteger.ZERO;
        }
    }
}
